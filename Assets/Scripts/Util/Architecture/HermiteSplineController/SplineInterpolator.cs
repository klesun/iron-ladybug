...
public class SplineInterpolator : MonoBehaviour {
    ...
    public void SetAutoCloseMode(float joiningPointTime)
    {
        ...
        var vInitDir = (mNodes[1].Point - mNodes[0].Point).normalized;
        var vEndDir = (mNodes[mNodes.Count - 2].Point - mNodes[mNodes.Count - 1].Point).normalized;
        var firstLength = (mNodes[1].Point - mNodes[0].Point).magnitude;
        var lastLength = (mNodes[mNodes.Count - 2].Point - mNodes[mNodes.Count - 1].Point).magnitude;

        var firstNode = new SplineNode(mNodes[0] as SplineNode);
        firstNode.Point = mNodes[0].Point + vEndDir * firstLength;

        var lastNode = new SplineNode(mNodes[mNodes.Count - 1] as SplineNode);
